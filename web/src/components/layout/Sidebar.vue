  ...
  <aside class="fixed left-0 top-0 bottom-0 w-64 bg-dark-800 border-r border-dark-600 flex flex-col z-40">
    ...
    <nav class="flex-1 overflow-y-auto py-4">
      ...
      <div class="px-3 space-y-1">
        ...
        >
          <!-- Icons -->
          <div v-if="item.icon === 'home'" class="relative flex-shrink-0">
            ...
            <div
              ...
              class="absolute -top-2 -right-2 min-w-4 h-4 px-1 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center"
            >
          ...
          <div v-else-if="item.icon === 'users-group'" class="relative flex-shrink-0">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
            </svg>
            <!-- New parties badge -->
            <div
              v-if="notificationsStore.newPartiesCount > 0"
              class="absolute -top-2 -right-2 min-w-4 h-4 px-1 rounded-full bg-green-500 text-white text-[10px] font-bold flex items-center justify-center"
            >
              {{ notificationsStore.newPartiesCount > 99 ? '99+' : notificationsStore.newPartiesCount }}
            </div>
          </div>

          <div v-else-if="item.icon === 'chat'" class="relative flex-shrink-0">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
            <!-- Unread badge -->
            <div
              v-if="channelsStore.totalUnreadCount > 0"
              class="absolute -top-2 -right-2 min-w-4 h-4 px-1 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center"
            >
